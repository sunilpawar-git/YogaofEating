...
struct SmileyRealityView: View {
    ...
    var body: some View {
        RealityView { content in
            ...
            if abs(currentScale - state.scale) < 0.01 {
            ...
            let targetScale = Float(state.scale)
                ...
                // Animate smoothly
                smiley.move(to: Transform(scale: [targetScale, targetScale, targetScale]),
                            relativeTo: nil,
                            duration: 0.5,
                ...
                if let model = smiley as? ModelEntity {
                    model.model?.materials = [SimpleMaterial(color: colorForMood(state.mood), isMetallic: false)]
                    
                    if let mouth = model.findEntity(named: "Mouth") {
                        let mouthRotation: Float = {
                            switch state.mood {
                            case .serene: return .pi // Smile (upside down box is actually a smile if we position it right, or just use rotation)
                            case .neutral: return 0
                            case .overwhelmed: return .pi / 8 // Slightly tilted/distorted
                            }
                        }()
    ...
    private func colorForMood(_ mood: SmileyMood) -> UIColor {
        switch mood {
        case .serene: return .systemYellow
        case .neutral: return .systemOrange
        case .overwhelmed: return .systemRed
        }
    }
}
